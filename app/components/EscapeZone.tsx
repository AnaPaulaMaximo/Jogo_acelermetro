...
export default function EscapeZone() {
  ...
  const handleStartGame = () => {
    ...
    setPowerUps([]);
    setScenery([]);
    const initialSegments = Array.from({ length: Math.ceil(height / ROAD_SEGMENT_HEIGHT) + 2 }).map((_, i) => ({
        id: i,
  ...
  useEffect(() => {
    ...
    // Sensibilidade reduzida para a direção
    const steerSensitivity = 80;
    let newX = carPosition.x - accelerometerData.y * steerSensitivity;
    ...
    if (carSegment) {
        ...
        if (newX > carSegment.x + carSegment.width - CAR_SIZE) newX = carSegment.x + carSegment.width - CAR_SIZE;
    }

    const accelSensitivity = 0.1;
    let newSpeed = speed + (accelerometerData.x + 0.5) * accelSensitivity;
    if (accelerometerData.x < -0.8) {
        newSpeed = Math.max(INITIAL_SPEED, speed - 0.5);
    }
    if (newSpeed > MAX_SPEED) newSpeed = MAX_SPEED;
    if (newSpeed < INITIAL_SPEED) newSpeed = INITIAL_SPEED;

    setSpeed(newSpeed);
    setCarPosition({ ...carPosition, x: newX });

    Animated.spring(carTilt, {
      toValue: accelerometerData.y * -30,
      friction: 5,
      useNativeDriver: true,
    }).start();

  }, [accelerometerData, gameState]);
  ...
  useEffect(() => {
    ...
    const gameLoop = setInterval(() => {
      ...
      setObstacles(prev => prev.map(o => ({ ...o, y: o.y + speed })).filter(o => o.y < height + ZOMBIE_SIZE));
      ...
      setRoadSegments(prev => {
        ...
        if (lastSegment && lastSegment.y > -ROAD_SEGMENT_HEIGHT) {
            const difficulty = Math.min(score / 10000, 1);
            ...
            const newWidth = width * (0.6 - (0.3 * difficulty));
            const curveStrength = newCurve.direction * (2 + 4 * difficulty);
            ...
            if (Math.random() < 0.2) {
                ...
                const treeX = side > 0 ? newSegment.x + newSegment.width + Math.random() * 50 : newSegment.x - 50 - Math.random() * 50;
                setScenery(prevScenery => [...prevScenery, {id: Math.random(), x: treeX, y: newSegment.y, size: Math.random() * 40 + 40}]);
            }
      ...
      if (currentSegmentForSpawning) {
        if (Math.random() < 0.05) {
            const spawnX = currentSegmentForSpawning.x + Math.random() * (currentSegmentForSpawning.width - ZOMBIE_SIZE);
            setObstacles(prev => [...prev, { id: Math.random(), x: spawnX, y: -ZOMBIE_SIZE }]);
        }
        if (Math.random() < 0.01) {
  ...
  useEffect(() => {
    ...
    obstacles.forEach(obstacle => {
      const obstacleRect = { x: obstacle.x, y: obstacle.y, width: ZOMBIE_SIZE, height: ZOMBIE_SIZE };
  ...
  return (
    <View style={styles.container}>
      ...
      {obstacles.map(o => (
        <Image key={o.id} source={require('../../assets/images/zombie.png')} style={[styles.item, { width: ZOMBIE_SIZE, height: ZOMBIE_SIZE, left: o.x, top: o.y }]} />
      ))}
      {powerUps.map(p => (
      ...
      <Animated.Image
        ...
        style={[
          styles.car,
          {
            left: carPosition.x,
            top: carPosition.y,
            transform: [{ rotateZ: carTilt.interpolate({ inputRange: [-30, 30], outputRange: ['-15deg', '15deg'], extrapolate: 'clamp' }) }],
          },
        ]}
      />
    </View>
